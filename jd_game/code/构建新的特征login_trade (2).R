...
data_login = read.csv('t_login.csv')
...
trade_idHour = summarise(data_by_idHour, 
                ...
                is_risk = any(is_risk == 1),          #是否有风险(只要出现就视为有风险)
...
struc_feature_login_by_idDay = function(data_login){
  ...
  #最终的整理交易数据
  struc_data1 = summarise(data_login_by_idDay, 
                          timestamp = mean(timestamp),       #时间戳（平均时间）
                          login_count = n(),                   #登录总次数
  ...
  #--合并数据集(横向合并)
  struc_data = merge(struc_data1,struc_data2,by=c("id",'day'),all =TRUE)
  #-对na位置进行填充
...
#--将时间戳改为登录平均时刻的时间戳
names(login_by_idDay)[3] = 'login_timestamp'    #重命名
...
#-当前交易
index_trade = which(!is.na(login_trade3$is_risk))  #所有交易的index
#-最后一次登录数据
#1.先所有交易的前一次信息为登录，即使前面是交易也无所谓（交易的登录也是最近的填补的）
#2.寻找前1小时的登录
#--1.先所有交易的前一次信息为登录（交易的index，对应该次交易的所有信息）
last_login = data.frame(login_trade3_index = index_trade, login_trade3[index_trade-1,c('id','hour','log_id')])
#---连接对应的平均登录信息
#-增加一个day变量
...
#--2.连接交易、登录数据
trade_full = login_trade3[tradeId_login_full$login_trade3_index, c('id','time','timestamp','trade_count','is_risk')]
...
trade_login_full = cbind(trade_full,tradeId_login_full[,names_use])  #--------------


#write.csv(trade_login_full,'trade_login_full2.csv',row.names = FALSE)
