...
login_trade3_6=login_trade3  #6月份以前的登录数据
...
trade_login_full_temp = cbind(trade_full,tradeId_login_full[,-c(1:5)])  #--------------

#------3.连接ip数据
data_login_ip = read.csv('login_by_IP_7.csv')
names(data_login_ip)[-1] <- paste0(names(data_login_ip)[-1],'_ip') #重命名

trade_login_full = merge(trade_login_full_temp, data_login_ip,
                         by=c("ip"),all.x =TRUE)


#write.csv(trade_login_full,'trade_login_full_T_ip.csv',row.names = FALSE)
#write.csv(trade_login_full,'trade_login_full_T.csv',row.names = FALSE)
